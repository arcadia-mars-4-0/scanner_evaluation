# Ground Truth Scanner Dev Workflow
...
- [data/dev_t5_files.csv](/home/jeffm/projects/scanner_evaluation/scanner_dev/ground_truth/data/dev_t5_files.csv): source-of-truth manifest for the current corpus
- [build_dataset.py](/home/jeffm/projects/scanner_evaluation/scanner_dev/ground_truth/build_dataset.py): stages transcripts and generates merged metadata
- [scout.yaml](/home/jeffm/projects/scanner_evaluation/scanner_dev/ground_truth/scout.yaml): Scout config for this staging area
...
```bash
uv run python scanner_dev/ground_truth/build_dataset.py
```
...
```bash
uv run python scanner_dev/ground_truth/build_dataset.py --validated-only
```

Only specific `Eval` values:

```bash
uv run python scanner_dev/ground_truth/build_dataset.py --eval mle_bench_dev --eval mle_bench_low
```

Only specific methods:

```bash
uv run python scanner_dev/ground_truth/build_dataset.py --method default
```

Filters can be combined:

```bash
uv run python scanner_dev/ground_truth/build_dataset.py --validated-only --method default
```
...
```bash
uv run python scanner_dev/ground_truth/build_dataset.py \
  --manifest scanner_dev/ground_truth/data/contamination_round2.csv
```
